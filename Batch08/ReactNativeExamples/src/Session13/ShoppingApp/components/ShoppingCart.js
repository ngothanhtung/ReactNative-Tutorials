...
export default function ShoppingCart() {
  ...
  const renderItem = ({ item }) => {
    ...
    let sum = numeral((item.quantity * item.product.price * (100 - item.product.discount)) / 100).format('0,0');
  ...
  return (
    <View>
      ...
      <View style={{ backgroundColor: '#b2bec3', height: 1, marginVertical: 4 }} />
      <Text style={{ fontWeight: '700', color: '#d63031', textAlign: 'right' }}>Total: {getTotal()}</Text>
    </View>
  );
}
