...
export default class GetUsers extends Component {
	...
	renderUserItem = ({ item, index }) => {
		return (
			<View style={{ padding: 8, flexDirection: 'row' }}>
				<View>
					<Image style={{ width: 60, height: 60, borderRadius: 30 }} source={{ uri: 'https://i.pravatar.cc/150?u=' + item.email }} />
				</View>
	...
	render() {
		return <View style={{ flex: 1 }}>{this.state.loading === false && <FlatList data={this.state.users} renderItem={this.renderUserItem} keyExtractor={(item, index) => 'user_' + index.toString()} />}</View>;
	}
}
